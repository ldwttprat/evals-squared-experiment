...
Methodology, prompts, and divergence logs from the *evals²* experiment, which extends the cascade modeling first developed in [*The Research Risk Cascade: Why Even '90% Accurate' AI Tools Break Pipelines*](https://lindseydewittprat.substack.com/p/the-research-risk-cascade-why-even) (DeWitt Prat, January 2026). The experiment was presented at the [User Interviews webinar](https://www.userinterviews.com/events/evaluating-ai-across-the-research-pipeline) on 31 March 2026, and is published in *Finding Your Blue Dot: Evaluating AI Across the Research Pipeline* (*ResearchOps Review*, May 2026) — [link TK].

The experiment runs three five-minute podcast clips through three transcription tools, four summarization models, and one analysis model — and tracks every place the outputs diverge from the source and from each other. The goal: a worked example of how errors compound across an AI-augmented research pipeline, and a reproducible method any researcher can run on their own source material in an afternoon.
...
The methodology in `EXPERIMENT.md` is designed to be portable. Pick one source — a recorded interview, a focus group, a podcast clip — and run it through whatever combination of tools your pipeline uses. Compare the outputs against each other and against the source. The shape of your cascade will come into view.
